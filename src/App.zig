...
/// A closed tab whose kitty image cache may still be borrowed by an
/// in-flight async job; deferred until the raster worker is idle.
pending_tab_cleanup: ?*Tab = null,
...
pub const AppStreamHandler = struct {
    ...
    pub fn vt(
        ...
        switch (action) {
            ...
            .clipboard_contents => app.setOsc52Clipboard(value.kind, value.data),
            .kitty_clipboard => {
...
pub fn init(
    ...
    var startup_padding = physicalPadding(config, 120);
    startup_padding.top += tab_bar_height;
    ...
    first_tab.stream.handler.terminal_handler.effects = effects;
...
fn effectTitleChanged(handler: *Handler) void {
    ...
    const title = tb.term.getTitle() orelse return;
    tb.app.window.toplevel.setTitle(title.ptr);
}
...
fn setColorScheme(self: *App, color_scheme: vt.device_status.ColorScheme, report: bool) void {
    ...
    if (report and self.tab().term.modes.get(.report_color_scheme)) self.sendColorSchemeReport(self.tab());
}
...
pub fn deinit(self: *App) void {
    ...
    self.async_job.deinit(self.alloc, &self.active.kitty_cache);
    ...
    if (self.pending_tab_cleanup) |closing| {
        self.pending_tab_cleanup = null;
        closing.deinit();
    }
...
pub fn run(self: *App) !void {
    ...
    while (self.window.running and (self.anyChildAlive() or self.hold)) {
        ...
        // Free a closed tab once the raster worker no longer borrows its
        // kitty image cache.
        if (self.pending_tab_cleanup) |closing| {
            if (self.async_raster == null or !self.async_raster.?.busy()) {
                self.pending_tab_cleanup = null;
                closing.deinit();
            }
        }
...
fn closeTab(self: *App) void {
    ...
    if (self.async_raster != null and self.async_raster.?.busy()) {
        self.pending_tab_cleanup = closing;
    } else {
        closing.deinit();
    }
...
fn applyConfig(self: *App, new_config: Config) !void {
    ...
    if (new_config.image_storage_limit != self.config.image_storage_limit) {
        self.tab().term.setKittyGraphicsSizeLimit(self.alloc, new_config.image_storage_limit);
    }
...
fn applyColorDefaultsForConfig(self: *App, config: Config) void {
    ...
    self.tab().term.colors.background.default = colors.background.default;
    self.tab().term.colors.foreground.default = colors.foreground.default;
    self.tab().term.colors.cursor.default = colors.cursor.default;
    self.tab().term.colors.palette.changeDefault(colors.palette.original);
...
fn syncSynchronizedOutput(self: *App, tb: *Tab) void {
    const enabled = tb.term.modes.get(.synchronized_output);
    if (enabled == tb.sync_output) return;

    tb.sync_output = enabled;
    if (enabled) {
...
fn fireSyncOutputReset(self: *App) void {
    ...
    for (self.tabs.items) |tb| {
        if (!tb.term.modes.get(.synchronized_output)) continue;
...
fn pumpKittyClipboard(self: *App) void {
    while (self.kitty_clipboard.front()) |request| switch (request.*) {
        .status => |*status| {
            self.writeKittyClipboardStatus(self.tab(), status.op, status.id, status.terminator, status.status);
            self.kitty_clipboard.pop();
        },
        .write => |*write| {
            self.kitty_clipboard.prepareWrite(write);
            const target = clipboardTarget(write.committed.loc) orelse {
                self.writeKittyClipboardStatus(self.tab(), .write, write.committed.id, write.terminator, .ENOSYS);
                self.kitty_clipboard.pop();
                continue;
            };
            ...
            self.writeKittyClipboardStatus(self.tab(), .write, write.committed.id, write.terminator, status);
            self.kitty_clipboard.pop();
        },
        .read => |*read| {
            if (read.started) return;
            self.kitty_clipboard.prepareRead(read) catch {
                self.writeKittyClipboardStatus(self.tab(), .read, read.id, read.terminator, .EIO);
                self.kitty_clipboard.pop();
                continue;
            };
            if (read.paste) |paste| {
...
fn failStartedKittyRead(self: *App, status: vt.kitty.clipboard.Status) void {
    ...
    if (!read.started) return;
    self.writeKittyClipboardStatus(self.tab(), .read, read.id, read.terminator, status);
    self.kitty_clipboard.pop();
}
...
fn writeTerminalPaste(
    ...
    switch (source) {
        .clipboard => |location| if (self.tab().term.modes.get(.kitty_paste_events)) {
    ...
    _ = self.tab().stream.handler.terminal_handler.paste(.{
        .source = source,
...
fn scrollLines(self: *App, lines_down: i32) void {
    ...
    if (self.tab().term.flags.mouse_event != .none) {
    ...
    if (self.tab().term.screens.active_key == .alternate) {
...
fn startAsyncRender(self: *App) !AsyncRenderStart {
    ...
    if (!frozen) {
        ...
        errdefer self.freeTabBarSnapshot(new_bar);
        const bar_dirty = self.async_job.tab_bar_height != self.tab_bar_height or
            !tabBarEqual(self.async_job.tab_bar, new_bar);
        overlay_dirty = overlay_dirty or bar_dirty;
        try self.async_job.replaceTabBar(self.alloc, new_bar, self.tab_bar_height);
        self.freeTabBarSnapshot(new_bar);
        const kitty_dirty = self.tab().term.screens.active.kitty_images.dirty;
        ...
        if (has_kitty_graphics) {
            ...
            self.async_job.releaseKitty(self.alloc, &self.tab().kitty_cache);
        }
        self.tab().kitty_cache.sweep(self.alloc);
    ...
    async_raster.submit(.{
        ...
        .tab_bar_height = self.async_job.tab_bar_height,
...
fn resizeForConfig(self: *App, width: u31, height: u31, config: Config) anyerror!void {
    ...
    var padding = physicalPadding(config, self.window.scale120);
    padding.top += self.tab_bar_height;
