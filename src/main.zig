...
fn gui(init: std.process.Init, cli: CliOptions) !void {
    ...
    var command = try buildCommand(arena, config, init.minimal.environ, cli.command_mode, cli.command);
...
fn buildCommand(
    ...
    if (command.len > 0) switch (mode) {
        ...
        .shell => |value| {
            if (try buildSimpleShellCommand(arena, environ, value)) |simple| return simple;
            path = "/bin/sh";
...
fn buildSimpleShellCommand(
    ...
    return .{
        .path = try App.resolveCommandPath(arena, environ, parsed[0]),
